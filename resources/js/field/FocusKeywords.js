...
export default class FocusKeywords {
	...
	constructor (namespace, SEO) {
		...
		// Setup regular title field
		this.titleField = document.getElementById("title");

		// Setup regular slug field
	...
	initWatcher () {
		...
		this.mo = new MutationObserver(debounce(this.recalculateKeyword, this.recaculateTimeout));

		this.startObserving();

		// Run keyword calculation on regular title field
		this.titleField.addEventListener('input', debounce(this.recalculateKeyword, this.recaculateTimeout));

		this.slugField.addEventListener('input', debounce(this.recalculateKeyword, this.recaculateTimeout));
	}
	 ...
	 */
	recalculateKeyword = () => {
		// console.log("recalculateKeyword");
	...
	onKeywordRemoveClick = e => {
		// console.log("onKeywordRemoveClick");
		e.preventDefault();
		e.stopPropagation();
